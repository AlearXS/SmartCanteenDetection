...
def process_video_camera3(video_path, canteen_name):
    ...
    # 初始化2个撞线polygon(下)
    list_pts_blue = [[557, 254], [557, 285], [623, 293], [813, 261], [861, 176], [833, 160], [792, 235], [624, 268]]
    ...
    # 填充第二个polygon（上）
    mask_image_temp = np.zeros((674, 1200), dtype=np.uint8)
    list_pts_yellow = [[557,299], [557,330], [632,337], [864,299], [899,195], [879,184], [836,275], [624,313]]
...
def process_video_camera2(video_path):
    ...
    # 初始化2个撞线polygon(blue)
    list_pts_blue = [[341, 654], [371, 664], [482, 447], [494, 380]]
    ndarray_pts_blue = np.array(list_pts_blue, np.int32)
    polygon_blue_value_1 = cv2.fillPoly(mask_image_temp, [ndarray_pts_blue], color=1)
    polygon_blue_value_1 = polygon_blue_value_1[:, :, np.newaxis]
    ...
    # 填充第二个polygon（yellow）
    mask_image_temp = np.zeros((674, 1200), dtype=np.uint8)
    list_pts_yellow = [[398, 660], [431, 667], [549, 404],  [517, 401]]
    ndarray_pts_yellow = np.array(list_pts_yellow, np.int32)
    polygon_yellow_value_2 = cv2.fillPoly(mask_image_temp, [ndarray_pts_yellow], color=2)
    polygon_yellow_value_2 = polygon_yellow_value_2[:, :, np.newaxis]
...
def process_video(video_path, process_func):
    down_count, up_count = process_func(video_path)
...
if __name__ == '__main__':
    ...
    # 遍历视频文件列表，为每个视频创建一个线程
    for video_path in video_paths:
        if 'test3.mp4' in video_path:
            t = threading.Thread(target=process_video_camera3, args=(video_path,))
        elif 'test2.mp4' in video_path:
            t = threading.Thread(target=process_video_camera2, args=(video_path,))
        else:
            # 如果有其他视频文件，可以在这里添加处理逻辑
            print(f"Unhandled video file: {video_path}")
            continue
